...
class Hub:
    ...
    def __init__(self):
        colour = p_load(open(Path('data/data/colour_data.bat'), 'rb'))
        self.custom_colour = p_load(
            open(Path('data/data/custom_colour_data.bat'), 'rb'))
        colour_a, colour_b, colour_c = self.custom_colour
        self.colour_a, self.colour_b, self.colour_c = str(
            colour_a), str(colour_b), str(colour_c)
        ...
        self.user_level = p_load(open(Path('data/data/user_level.bat'), 'rb'))
        self.sound = p_load(open(Path('data/data/sound_data.bat'), 'rb'))
        self.ui_button_pos = p_load(open(Path('data/data/ui_button_data.bat'), 'rb'))
        self.max_levels = 27
        self.scroll_y = 220
    ...
    def press_button(self, click=False):
        ...
        if arrow_right_rect.collidepoint(mouse_pos):
            if self.sound and click and self.max_page_no != 3:
                click_sound.play()
            return 'next'
        if arrow_left_rect.collidepoint(mouse_pos):
            if self.sound and click:
                click_sound.play()
            return 'previous'
    ...
    def locked_levels(self, slide):
        ...
        for index, rectangles in enumerate(level_button_rects):
            ...
            if level_num > self.user_level:
                screen.blit(
                    locked_level, (rectangles.topleft[0]+slide, rectangles.topleft[1]))
        ...
        if levelnames:
            ...
            for index, level_name in enumerate(levelnames):
                ...
                text_surface = game_font.render(
                    level_name[:-5], True, (8, 100, 125))
                text_rect = text_surface.get_rect(
                    center=(slide+300, temp_y_slide+30))
                ...
                if self.scroll_now:
                    ...
                    self.custom_level_rects.append(
                        [rect_one, rect_two, rect_three])
    ...
    def create_custom_level(self):
        ...
        while waiting:
            name = Path('data/custom levels/untitled-'+str(num)+'X.bat')
        ...
        shut_copy(Path('data/data/temp_level.bat'), name)
    ...
    def del_custom_level(self, levelnames, index_bar):
        ...
        level_name = Path('data/custom levels/' + str(levelnames[index_bar]))
    ...
    def del_confirm_page(self, levelnames):
        ...
        while True:
            ...
            del_page_animation = pygame.transform.scale(
                confirm_page, (size*4, size*2))
            del_page_animation_rect = del_page_animation.get_rect(
                center=(380, 380))
            ...
            if not close and size != 100:
                ...
                the_text = 'Delete "' + \
                    str(levelnames[self.del_confirm-1][:-5]) + '" ?'
                text_surface = normal_font.render(
                    the_text, True, (8, 100, 125))
                    ...
                    if button.collidepoint(mouse_pos):
                        ...
                        if click:
                            if self.sound:
                                click_sound.play()
                            if index_button == 0:
                                go_to_hub = self.play_custom_level(
                                    levelnames, index_bar)
    ...
    def load_custom_levels(self):
        ...
        self.levelnames = next(os_walk(Path('data/custom levels')))[2]
    ...
    def set_sound(self, click=False):
        ...
        if sound_on_rect.collidepoint(mouse_pos) and self.sound:
            ...
            if click:
                click_sound.play()
                self.sound = False
                p_dump(self.sound, open(Path('data/data/sound_data.bat'), 'wb'))
        ...
        if slid and click:
            ...
            for index, rect in enumerate(colour_rect):
                if rect.collidepoint(mouse_pos):
                    ...
                    p_dump((self.colour, self.default_colour),
                           open(Path('data/data/colour_data.bat'), 'wb'))
    ...
    def change_ui_button_pos(self, change, cog_slide=0):

        small_font.underline = True
        pos_text = small_font.render(self.ui_button_pos, True, (230, 230, 255))
        pos_text_rect = pos_text.get_rect(center=(440+cog_slide, 660))
        small_font.underline = False
        mouse_pos = pygame.mouse.get_pos()

        if change:
            if pos_text_rect.collidepoint(mouse_pos):
                if self.sound: click_sound.play()

                if self.ui_button_pos == 'left':
                    self.ui_button_pos = 'right'
                elif self.ui_button_pos == 'right':
                    self.ui_button_pos = 'None'
                elif self.ui_button_pos == 'None':
                    self.ui_button_pos = 'left'

                p_dump(self.ui_button_pos, open(Path('data/data/ui_button_data.bat'), 'wb'))

        else:
            if pos_text_rect.collidepoint(mouse_pos):
                highlighted_button = pygame.Surface(pos_text_rect.size, pygame.SRCALPHA)
                highlighted_button.fill((0, 0, 255))
                highlighted_button.set_alpha(50)

                return pos_text, pos_text_rect, highlighted_button
            return pos_text, pos_text_rect, None
    ...
    def settings_code(self):
        ...
        while True:
            ...
            if self.colour_input:
                ...
                save_colour = True
                p_dump(self.custom_colour, open(
                    Path('data/data/custom_colour_data.bat'), 'wb'))
            ...
            # ui buttons
            ui_pos_text, ui_pos_text_rect, highlighted_button = self.change_ui_button_pos(False, cog_slide)
            ui_button_text_rect = ui_button_text.get_rect(center=(320+cog_slide, 660))
            screen.blit(ui_button_text, ui_button_text_rect)
            screen.blit(ui_pos_text, ui_pos_text_rect)
            if highlighted_button:
                screen.blit(highlighted_button, (ui_pos_text_rect))
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP and cog_slide == 0:
                    if event.button == 1:
                        ...
                        self.set_sound(True)
                        self.change_ui_button_pos(True)
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    self.custom_colour = int(self.colour_a), int(
                        self.colour_b), int(self.colour_c)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP and lev_slide == 0:
                    ...
                    if event.button == 1:
                        ...
                        go_to_hub = self.custom_level_button_click(
                            self.levelnames, click=True)
...
game_icon = pygame.image.load(Path('data/assets/fill-color.png')).convert_alpha()
pygame.display.set_icon(game_icon)

hub = Hub()

game_font = pygame.font.Font(Path('data/font/IndieFlower-Regular.ttf'), 40)
normal_font = pygame.font.Font(Path('data/font/arial.ttf'), 30)
...
# Assets
background = pygame.image.load(Path('data/assets/background.png')).convert_alpha()
space = pygame.image.load(Path('data/assets/space.png')).convert_alpha()
level_button = pygame.image.load(
    Path('data/assets/level_button.png')).convert_alpha()
level_button_template = pygame.image.load(
    Path('data/assets/level_button_template.png')).convert_alpha()
highlighted_level_button = pygame.image.load(
    Path('data/assets/highlighted_level_button.png')).convert_alpha()
hub_button = pygame.image.load(Path('data/assets/hub_button.png')).convert_alpha()
hub_button_selected = pygame.image.load(
    Path('data/assets/hub_button_selected.png')).convert_alpha()
arrow_right = pygame.image.load(Path('data/assets/arrow_right.png')).convert_alpha()
arrow_left = pygame.transform.rotate(arrow_right, 180)
highlighted_arrow_right = pygame.image.load(
    Path('data/assets/highlighted_arrow_right.png')).convert_alpha()
highlighted_arrow_left = pygame.transform.rotate(highlighted_arrow_right, 180)
level_page_1 = pygame.image.load(
    Path('data/assets/level_page_1.png')).convert_alpha()
level_page_2 = pygame.image.load(
    Path('data/assets/level_page_2.png')).convert_alpha()
level_page_3 = pygame.image.load(
    Path('data/assets/level_page_3.png')).convert_alpha()
no_continue = pygame.image.load(Path('data/assets/no_continue.png')).convert_alpha()
locked_level = pygame.image.load(
    Path('data/assets/locked_level.png')).convert_alpha()
custom_level_button = pygame.image.load(
    Path('data/assets/custom_level_button.png')).convert_alpha()
scroll_window = pygame.image.load(
    Path('data/assets/scroll_window.png')).convert_alpha()
background_window = pygame.image.load(
    Path('data/assets/background_window.png')).convert_alpha()
custom_button_selected = pygame.image.load(
    Path('data/assets/custom_button_selected.png')).convert_alpha()
custom_side_selected = pygame.image.load(
    Path('data/assets/custom_side_selected.png')).convert_alpha()
red_trash_can = pygame.image.load(
    Path('data/assets/red_trash_can.png')).convert_alpha()
edit_pen = pygame.image.load(Path('data/assets/edit_pen.png')).convert_alpha()
confirm_page = pygame.image.load(
    Path('data/assets/confirm_page.png')).convert_alpha()
menu_background = pygame.image.load(
    Path('data/assets/menu_background.png')).convert_alpha()
del_yes_no = pygame.image.load(Path('data/assets/del_yes_no.png')).convert_alpha()
del_yes_no_selected = pygame.image.load(
    Path('data/assets/del_yes_no_selected.png')).convert_alpha()
create_level_button = pygame.image.load(
    Path('data/assets/create_level_button.png')).convert_alpha()
create_level_selected = pygame.image.load(
    Path('data/assets/create_level_selected.png')).convert_alpha()
help_icon = pygame.image.load(Path('data/assets/help_icon.png')).convert_alpha()
help_box = pygame.image.load(Path('data/assets/help_box.png')).convert_alpha()
help_base = pygame.image.load(Path('data/assets/help_base.png')).convert_alpha()
help_window = pygame.image.load(Path('data/assets/help_window.png')).convert_alpha()
help_page_text = pygame.image.load(Path('data/assets/help_text.png')).convert_alpha()
info_page_text = pygame.image.load(Path('data/assets/info_text.png')).convert_alpha()
info_icon = pygame.image.load(Path('data/assets/info_icon.png')).convert_alpha()
option_background = pygame.image.load(
    Path('data/assets/option_background.png')).convert_alpha()
slider_bob = pygame.image.load(Path('data/assets/slider_bob.png')).convert_alpha()
slider_selected = pygame.image.load(
    Path('data/assets/slider_selected.png')).convert_alpha()
slider = pygame.image.load(Path('data/assets/slider.png')).convert_alpha()
colour_selected = pygame.image.load(
    Path('data/assets/colour_selected.png')).convert_alpha()
colour_input_box = pygame.image.load(
    Path('data/assets/colour_input_box.png')).convert_alpha()
colour_input_box_selected = pygame.image.load(
    Path('data/assets/colour_input_box_selected.png')).convert_alpha()
no_custom_level_text = pygame.image.load(
    Path('data/assets/no_custom_level_text.png')).convert_alpha()

info_stickman = pygame.image.load(
    Path('data/assets/stickman/info_stickman.png')).convert_alpha()
help_stickman = pygame.image.load(
    Path('data/assets/stickman/help_stickman.png')).convert_alpha()
quit_stickman = pygame.image.load(
    Path('data/assets/stickman/quit_stickman.png')).convert_alpha()
options_stickman = pygame.image.load(
    Path('data/assets/stickman/options_stickman.png')).convert_alpha()
levels_stickman = pygame.image.load(
    Path('data/assets/stickman/levels_stickman.png')).convert_alpha()
continue_stickman = pygame.image.load(
    Path('data/assets/stickman/continue_stickman.png')).convert_alpha()
custom_lev_stickman = pygame.image.load(
    Path('data/assets/stickman/custom-lev_stickman.png')).convert_alpha()
...
ui_button_text = small_font.render('UI Buttons Position:', True, (230, 230, 255))
...
pop_button = pygame.mixer.Sound(Path('data/sound/pop_button.wav'))
