...
def play_game(level_name, default_one=False, editable=False):
    ...
    class MainGame:
        ...
        def __init__(self, level_name, default_one=False):
            ...
            if colour[1]:
                ...
                self.colour = p_load(open(Path('data/data/custom_colour_data.bat'), 'rb'))
            self.boxes_passed = 0
            self.won  =False
            self.menu = False
            ...
            self.level = p_load(open(Path(level_name), 'rb'))
        ...
        def game_menu(self):
            ...
            while True:
                ...
                if not self.pause_menu and self.current_level + 1 > self.user_level:
                    p_dump(self.user_level+1, open(Path('data/data/user_level.bat'), 'wb'))
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and menu_slide == 0:
                        if self.check_ui_menu_button(True) and self.pause_menu:
                            slide_up = True
                            break
    ...
    ui_arrow = pygame.image.load(Path('data/assets/ui_buttons/ui_arrow.png')).convert_alpha()
    ...
    brick_background = pygame.image.load(Path('data/assets/brick_background.png')).convert_alpha()
    menu_surface = pygame.image.load(Path('data/assets/menu_surface.png')).convert_alpha()
    ...
    pause_menu_resume = pygame.image.load(Path('data/assets/pause_menu_resume.png')).convert_alpha()
    go_back_menu = pygame.image.load(Path('data/assets/go_back_menu.png')).convert_alpha()
    menu_next_level = pygame.image.load(Path('data/assets/menu_next_level.png')).convert_alpha()
    edit_icon = pygame.image.load(Path('data/assets/edit_icon.png')).convert_alpha()
    edit_box = pygame.image.load(Path('data/assets/edit_box.png')).convert_alpha()
    ui_menu_button = pygame.image.load(Path('data/assets/ui_buttons/drop_menu_button.png')).convert_alpha()
    ...
    hit_sound = pygame.mixer.Sound(Path('data/sound/hit_sound.wav'))
    win_sound = pygame.mixer.Sound(Path('data/sound/win_sound.wav'))
